...
var gun = Gun({
	file: 'data/master-data.json',
  peers: ['http://localhost:9001/gun']
});
...
function acceptConnection( connection ) {
    ...
    console.log( "new connection", connection._ultron.id )
    ...
    connection.on('message', function (msg) {
        ...
        msg = JSON.parse(msg)
        if ("forEach" in msg) msg.forEach(m => {
          console.log('1.gun message : ', m);
